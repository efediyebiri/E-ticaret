<nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
      ...
      <ul class="navbar-nav navbar-nav-right">
        ...
        <li class="nav-item nav-profile dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="profileDropdown">
            ...
            <a class="dropdown-item" href="{{route('anasayfa')}}" target="_blank" id="link">
                <i class="ti-settings text-primary"></i>
                Anasayfa
              </a>
            <a class="dropdown-item" href="{{route('cikis')}}">
              <i class="ti-power-off text-primary text-primary"></i>
              Çıkış
            </a>
          </div>
        </li>
  ...
  <script>
    // Bağlantıya tıklandığında yeni sayfa açma
    document.getElementById("link").addEventListener("click", function(event) {
      event.preventDefault(); // Bağlantının varsayılan davranışını engelle
      var url = this.getAttribute("href"); // Bağlantının URL'sini al
      window.open(url, "_blank"); // Yeni pencerede URL'yi aç
    });
    </script>
